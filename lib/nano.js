...
const AGENT_DEFAULTS = { keepAlive: true, maxSockets: 50, keepAliveMsecs: 30000, timeout: 60000 }
...
module.exports = exports = function dbScope (cfg) {
  ...
  function relax (opts, callback) {
    ...
    // this makes the connection timeout the same as request to prevent connection stalls in axios
    const requestTimeout = cfg.requestTimeout || defaultTimeout
    let timeout = null
    if (requestTimeout) {
      const source = CancelToken.source()
      timeout = setTimeout(() => {
        // call existing timeout logic
        source.cancel()
      }, requestTimeout) // make req and connection timeout the same for axios calls
